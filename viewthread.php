...
if(isset($_POST['submit']))
{
	...
	$sql = "INSERT INTO `threadlist` ( `title`, `description`, `tid`, `uid`  ) VALUES ( '$problem', '$describe', '$id', '$userid')";
	$resul = mysqli_query( $conn, $sql);
  ...
  if($ins) {
  	...
  	echo '
        ...
        <strong>Congrats!&nbsp</strong> Insertion Successfull.
  ...
  if($fail)
	{
		echo'
          <div class="alert alert-danger alert-dismissible fade show" role="alert">
        <strong>Error!&nbsp</strong> Insertion Uncessfull.
        <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
      </div>';
	}
...
<?php 
    ...
    $sql = " SELECT * FROM `thread` WHERE SNo=$id";
...
<figure class=" mt-4">
  ...
  <div class="mb-3">
    ...
    <div  class="form-text">Describe your problem in precise manner.</div>
  </div>
...
<figure class=" mt-4">
  <blockquote class=" text-center display-6">
    ...
    $sql = "SELECT * from threadlist where tid = '$id' and uid = '$userid'";
    $result = mysqli_query( $conn, $sql);
    ...
    $sno=$row['sno'];
      echo '<div class= " row mt-4 mb-4" style="margin-left:10%;">
      <div class="col-sm-1">
  <img class="align-self-center mt-2 mb-2 ml-4 rounded-circle" src="'.$userimg.'" alt= "Generic placeholder image" height="60px" width="60px" style="margin-left:35%; margin-top:40%;">
   ...
   <a href="comment.php?sid='.$sno.'"> <h4 class="mt-0 " >'.$row['title'].'</h4><p class="text-justify"></a>
    '.$row['description'].'</p>
